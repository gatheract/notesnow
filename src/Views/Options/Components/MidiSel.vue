  ...
  <div class="midiContainer">
    <div v-if="midiSupport">
      ...
      <div v-if="midiEnabled" class="midiControllerStatus">
        <div v-if="midiAvailable === MidiStatus.INITIALIZING">Initializing</div>
        <div v-else-if="midiAvailable === MidiStatus.AVAILABLE">
          <div>
            <el-select
              style="max-width: 300px;"
              v-on:change="inputSelected"
              :value="midiInputId"
        ...
        <div v-else-if="midiAvailable === MidiStatus.FAILED">{{$t('MidiSel.notAvailable')}}</div>
      </div>
    </div>
    <div class="notSupported" v-else>
      <span>{{$t('MidiSel.notSupported')}}</span>
      <br>
